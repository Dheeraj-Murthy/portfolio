...
(() => {
    ...
    // Game state
    const gravity = 0.9;
    const player = {x:60,y:260,w:40,h:60,vx:0,vy:0,onGround:false,jumpCount:0,facing:1,anim:'idle',frameTimer:0};
    const keys = {};
    let scrollX = 0;
    const platforms = [];
    const orbs = [];
    ...
    const spriteStore = { warrior: {}, orbImg: null, terminalImg: null, platformImg: null };

    // Load sprites from assets/warrior
    function loadSprites(callback){
        let toLoad = 0, done = 0;
        function check(){ if(done>=toLoad) callback && callback(); }
        // load warrior animations
        Object.keys(animConfig).forEach((k)=>{
            const cfg = animConfig[k];
            // main image
            toLoad++; (function(key,cfg){
                const img = new Image();
                img.src = './assets/warrior/' + cfg.file;
                img.onload = ()=>{ spriteStore.warrior[key] = {img:img, frames: cfg.frames, fps: cfg.fps}; done++; check(); };
                img.onerror = ()=>{ done++; check(); };
            })(k,cfg);
            // left-facing image (prefer explicit left image if provided)
            toLoad++; (function(key,cfg){
                const limg = new Image();
                limg.src = './assets/warrior/' + (cfg.leftFile || cfg.file);
                limg.onload = ()=>{ spriteStore.warrior[key + '_left'] = {img:limg, frames: cfg.frames, fps: cfg.fps}; done++; check(); };
                limg.onerror = ()=>{ done++; check(); };
            })(k,cfg);
        });
        // orb/terminal/platform images (optional)
        toLoad++; const orb = new Image(); orb.src = './assets/orb.png'; orb.onload=()=>{spriteStore.orbImg=orb; done++; check();}; orb.onerror=()=>{done++; check();};
        toLoad++; const term = new Image(); term.src = './assets/terminal.png'; term.onload=()=>{spriteStore.terminalImg=term; done++; check();}; term.onerror=()=>{done++; check();};
        toLoad++; const plat = new Image(); plat.src = './assets/platform.png'; plat.onload=()=>{spriteStore.platformImg=plat; done++; check();}; plat.onerror=()=>{done++; check();};
    ...
    function makeLevel(){
        ...
        platforms.push({x:-1000,y:H-60,w:6000,h:60, collisionOffset: 27});
        const spacing = 160;
        for(let i=0;i<14;i++){
    ...
    window.addEventListener('keydown', (e)=>{
        ...
        if(key==='arrowup' || key==='w' || key===' '){
            if(!lastJumpKey){
                if(player.onGround){
                    player.vy = -14; player.onGround=false; player.jumpCount = 1; setAnim('jump');
                } else if(player.jumpCount < 2){
                    player.vy = -15; player.jumpCount++; setAnim('jump');
                }
            }
            lastJumpKey = true;
        }
    ...
    function physicsUpdate(){
        // movement input
        if(keys['a'] || keys['arrowleft']){ player.vx = -4.2; player.facing = -1; if(player.onGround) setAnim('run'); }
        else if(keys['d'] || keys['arrowright']){ player.vx = 4.2; player.facing = 1; if(player.onGround) setAnim('run'); }
        ...
        if (!player.onGround) {
            if (player.vy > gravity * 0.1) { // Small threshold to be in 'fall' state
                setAnim('fall');
            } else if (player.vy < 0) {
        ...
        for(const p of platforms){
            ...
            if(collideRect(player.x,player.y,player.w,player.h, p.x, p.y, p.w, p.h)){
                ...
                if(player.vy >= 0 && (player.y + player.h) > (p.y + p.collisionOffset) && (player.y + player.h - player.vy) <= (p.y + p.collisionOffset)){
        ...
        for(const o of orbs){
            ...
            if(collideRect(player.x,player.y,player.w,player.h, o.x, o.y - o.r, o.r*2, o.r*2)){
                o.collected = true; showNotification(`Skill Unlocked: ${o.skill}`);
            }
        }

        // terminals interact
        if(keys['e']){
            if (!lastEKey){ for(const t of terminals){
    ...
    function draw(now, dt){
        ...
        g.addColorStop(0,'#1a2a4f'); g.addColorStop(1,'#102038');
        ...
        for(const p of platforms){
            ...
            else { ctx.fillStyle = '#08283f'; roundRect(ctx, px, py, p.w, p.h, 8); ctx.strokeStyle = 'rgba(139,92,246,0.14)'; ctx.lineWidth = 2; ctx.stroke(); }
        }
        ...
        for(const o of orbs){
            ...
            else { const grad = ctx.createRadialGradient(ox,oy,1,ox,oy,20); grad.addColorStop(0,'#ffd6a8'); grad.addColorStop(1,'#ff6b6b22'); ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(ox,oy,o.r,0,Math.PI*2); ctx.fill(); ctx.shadowColor = '#ff9f6a44'; ctx.shadowBlur = 12; ctx.fill(); ctx.shadowBlur = 0; }
        }
        ...
        for(const t of terminals){
            ...
            if(spriteStore.terminalImg) ctx.drawImage(spriteStore.terminalImg, tx, ty, t.w, t.h);
                else { ctx.fillStyle = t.contact ? '#ef6c00' : '#ffca28'; roundRect(ctx, tx, ty, t.w, t.h, 6); ctx.fillStyle='#021126'; ctx.font='14px sans-serif'; ctx.fillText(t.contact ? '✉' : '⌘', tx + t.w/2 - 6, ty + t.h/2 + 6); }
        }
        ...
        let imgObj = null, frames = 1, fps = 8;
        if(leftAnim && spriteStore.warrior[keyLeft] && spriteStore.warrior[keyLeft].img) { imgObj = spriteStore.warrior[keyLeft].img; frames = spriteStore.warrior[keyLeft].frames; fps = spriteStore.warrior[keyLeft].fps; }
        else if(spriteStore.warrior[key] && spriteStore.warrior[key].img) { imgObj = spriteStore.warrior[key].img; frames = spriteStore.warrior[key].frames; fps = spriteStore.warrior[key].fps; }
        ...
        if(imgObj && imgObj.complete && imgObj.naturalWidth !== 0){
            ...
            ctx.drawImage(imgObj, srcX, 0, sW, sH, px - (dw-player.w)/2, py - (dh-player.h), dw, dh);
        } else {
            // fallback rectangle
            ctx.fillStyle = '#f8f9fb'; roundRect(ctx, px, py, player.w, player.h, 6); ctx.strokeStyle = '#ff9f6a88'; ctx.lineWidth = 2; ctx.strokeRect(px, py, player.w, player.h);
        }
        ...
        for(let i = notifications.length - 1; i >= 0; i--){
            ...
            if(age > 3) { // Remove notifications after 3 seconds
    ...
    function initPlayerPos(){ player.x = 60; player.y = H - 220; player.vx=0; player.vy=0; player.jumpCount=0; scrollX=0; }
